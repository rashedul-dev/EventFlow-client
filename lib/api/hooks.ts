...
export function useAvailableSeats(eventId: string, options?: Partial<UseQueryOptions<{ data: Seat[] }>>) {
  return useQuery({
    ...
    enabled: !!eventId,
    refetchInterval: 5000, // Poll every 5 seconds for real-time availability
    ...options,
  })
}
...
export function useCreatePaymentIntent() {
  return useMutation({
    mutationFn: (data: { eventId: string; ticketTypeId: string; quantity: number }) =>
      apiClient.post<{ clientSecret: string; paymentIntentId: string }>("/payments/intent", data),
  })
}
...
export function useConfirmPayment() {
  ...
  return useMutation({
    mutationFn: (data: { paymentIntentId: string }) => apiClient.post("/payments/confirm", data),
    onSuccess: () => {
